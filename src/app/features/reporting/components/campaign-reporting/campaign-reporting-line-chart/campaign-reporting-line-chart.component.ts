...
@Component({
  selector: "app-campaign-reporting-line-chart",
  templateUrl: "./campaign-reporting-line-chart.component.html",
  styleUrls: ["./campaign-reporting-line-chart.component.scss"],
})
export class CampaignReportingLineChartComponent implements OnInit {
      ...
      .pipe(
        ...
        tap(([data, metrics]) => {
          ...
          this.lineChartLabels = _chain(data)
            .uniqBy("timestamp")
            .map((campaignDay) => {
              return campaignDay.timestamp;
            })
            .value();

          // build list of keywords
          const campaignList = _chain(data)
            .uniqBy("campaignId")
            .map((campaign) => {
          ...
          _each(campaignList, (campaign) => {
            _each(this.lineChartLabels, (date) => {
              ...
              const valuesForADay = _filter(data, (line) => {
                if (line.timestamp === date && line.campaignName === campaign) {
                  return true;
                }
              });
